...
$env:Path = "" #example C:\Program Files (x86)\1cv8\8.3.12.1595\bin
#указываем ip adress и порт для RAS.exe
$IpAddressAndPort = "" #example 192.168.1.218:1540
...
#задаем путь к глобальному логу скоипта
...

{
    ...
    Write-Output "FATAL ERROR - PowerShell check not passed" | Out-File "$globallog" -Append
    Break 
}
...
{ 
    ...
    Write-Host "Script terminated with error." -ForegroundColor Yellow
    Write-Output "FATAL ERROR - Ragent.exe service is running check not passed" | Out-File "$globallog" -Append
    Break 
}
...

{
    ...
    Write-Host "Script terminated with error." -ForegroundColor Yellow
    Write-Output "FATAL ERROR - Ragent.exe service is found check not passed" | Out-File "$globallog" -Append
    Break
}
...

{
    ...
    Write-Output $Error[0].Exception.Message 
    Write-Host "Script terminated with error." -ForegroundColor Yellow
    Write-Output "FATAL ERROR - RAS.exe is registered check not passed" | Out-File "$globallog" -Append
    Break
}
...

{
    ...
    Write-Output $Error[0].Exception.Message
    Write-Host "Script terminated with error." -ForegroundColor Yellow
    Write-Output "FATAL ERROR - Destination folder create check not passed" | Out-File "$globallog" -Append
    Break
}
...
if (Get-Content ($TempLogs| Select-Object -ExpandProperty "fullname") | where {($_ -match "error") -or ($_ -match "Ошибка")} |select PSPath) 
...
if (($infobases | Select-Object -ExpandProperty "name"| Out-String) -contains (get-childitem $destination -Recurse | Where-Object {(-Not $_.PSIscontainer -and $_.Length -gt 0) -and ($_.Name -match "$BackupDate"+('.dt')) } | Select-Object -ExpandProperty "fullname" | split-path -parent | split-path -leaf| Out-String))
